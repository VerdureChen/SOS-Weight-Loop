...
LOOP_CONFIG_PATH_NAME="${RUN_DIR}/run_configs/fact_weight_${QUERY_DATA_NAME}_loop_config_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $LOOP_CONFIG_PATH_NAME
TOTAL_LOG_DIR="${RUN_DIR}/run_logs/fact_weight_${QUERY_DATA_NAME}_loop_log_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $TOTAL_LOG_DIR
OUTPUT_DIR="${RUN_DIR}/../data_v2/loop_output/DPR/fact_weight_${QUERY_DATA_NAME}_loop_output_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $OUTPUT_DIR
...
do
  ...
  python ../rewrite_configs.py --method "${RETRIEVAL_MODEL_NAME}" \
                        ...
                        --stage "weight_init" \
                        --output_dir "${CONFIG_PATH}" \
                        --overrides '{"task":"set_fact_weight", "input_file": "'"${INPUT_FILE_PATH}"'", "output_file": "'"${OUTPUT_FILE_PATH}"'" ,
                        "elasticsearch_url": "'"${elasticsearch_url}"'", "normalize_embeddings": false,
