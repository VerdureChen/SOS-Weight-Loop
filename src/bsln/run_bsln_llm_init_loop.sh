...
TIMESTAMP=20250218065654

# concanate all the query data names
QUERY_DATA_NAME=$(IFS=_; echo "${QUERY_DATA_NAMES[*]}")

LOOP_CONFIG_PATH_NAME="${RUN_DIR}/run_configs/llm_weight_${QUERY_DATA_NAME}_loop_config_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $LOOP_CONFIG_PATH_NAME
TOTAL_LOG_DIR="${RUN_DIR}/run_logs/llm_weight_${QUERY_DATA_NAME}_loop_log_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $TOTAL_LOG_DIR
OUTPUT_DIR="${RUN_DIR}/../data_v2/loop_output/DPR/llm_weight_${QUERY_DATA_NAME}_loop_output_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $OUTPUT_DIR
...
do
  ...
  python ../rewrite_configs.py --method "${RETRIEVAL_MODEL_NAME}" \
                        ...
                        --stage "weight_init" \
                        --output_dir "${CONFIG_PATH}" \
                        --overrides '{"task":"set_llm_weight", "input_file": "'"${INPUT_FILE_PATH}"'", "output_file": "'"${OUTPUT_FILE_PATH}"'" ,
                        "elasticsearch_url": "'"${elasticsearch_url}"'", "normalize_embeddings": false,
