...
TOTAL_LOOP_NUM=10
#方法设置
RETRIEVAL_MODEL_NAME=llm-embedder # dpr contriever retromae all-mpnet bge-base llm-embedder bm25
FIT_TASK="2" # 0: retrieval 1: Diverse 2: LLM 3: Corr 9: None
...
#TIMESTAMP=20250222112331

# concanate all the query data names
QUERY_DATA_NAME=$(IFS=_; echo "${QUERY_DATA_NAMES[*]}")

LOOP_CONFIG_PATH_NAME="${RUN_DIR}/run_configs/fit_${FIT_TASK}_weight_${QUERY_DATA_NAME}_loop_config_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $LOOP_CONFIG_PATH_NAME
TOTAL_LOG_DIR="${RUN_DIR}/run_logs/fit_${FIT_TASK}_weight_${QUERY_DATA_NAME}_loop_log_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $TOTAL_LOG_DIR
OUTPUT_DIR="${RUN_DIR}/../data_v2/loop_output/DPR/fit_${FIT_TASK}_weight_${QUERY_DATA_NAME}_loop_output_${RETRIEVAL_MODEL_NAME}_total_loop_${TOTAL_LOOP_NUM}_${TIMESTAMP}"
mkdir -p $OUTPUT_DIR
...
do
  ...
  python ../rewrite_configs.py --method "${RETRIEVAL_MODEL_NAME}" \
                        ...
                        "added_documents": "'"${ADD_FILE_PATH}"'",
                        "new_add_fact_score_file": "'"${FACT_SCORE_PATH}"'", "fit_weights": "'"${FIT_TASK}"'"}'
...
#  fi
  ...
  CONFIG_PATH="${LOOP_CONFIG_PATH_NAME}/${RETRIEVAL_MODEL_NAME}_${QUERY_DATA_NAME}_alpha_index_update_weight_loop_${LOOP_NUM}.json"
  python ../rewrite_configs.py --method "${RETRIEVAL_MODEL_NAME}" \
                        --data_name "nq" \
                        --loop "${LOOP_NUM}" \
                        --stage "alpha_index" \
                        --output_dir "${CONFIG_PATH}" \
                        --overrides '{ "task": "update_weights", "index_exists": true, "new_text_file": "'"${OUTPUT_FILE_PATH}"'","output_files": ['"${OUTPUT_FILE_LIST}"'],
                        "page_content_column": "response", "index_add_path":"'"${TOTAL_LOG_DIR}"'", "index_pickle_path":"'"${TOTAL_LOG_DIR}"'", "query_files": ['"${QUERY_FILE_LIST}"'],
                        "elasticsearch_url": "'"${elasticsearch_url}"'", "normalize_embeddings": false}'
  wait

  echo "Running index update for ${RETRIEVAL_MODEL_NAME}"

  LOG_DIR="${TOTAL_LOG_DIR}/${RETRIEVAL_MODEL_NAME}_${QUERY_DATA_NAME}_alpha_index_update_weight_loop_${LOOP_NUM}.log"
  python embedding_index_incremental_corpus.py --config_file_path "$CONFIG_PATH" > "$LOG_DIR" 2>&1 &
