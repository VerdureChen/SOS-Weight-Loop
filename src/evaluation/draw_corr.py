...
for dataset in datasets:
    for loop in loops:
        # 读取TSV文件
        file_path = f'png_tsvs/corr/loop{loop}_plot_{dataset}_tsv.tsv'  # 替换成你的TSV文件路径
        ...
        sns.despine()
        plt.xlabel('Context Right Num') #, fontproperties=my_font, fontsize=30, fontweight='bold',)  # X轴标签
        plt.ylabel('Query Num')  # Y轴标签
        plt.title(f'Iteration {loop}', fontweight='bold')
        ...
        plt.show()  # 显示图表
        plt.savefig(f'png_tsvs/corr/{dataset}_{loop}_corr.png', bbox_inches='tight')  # 保存图表到文件
