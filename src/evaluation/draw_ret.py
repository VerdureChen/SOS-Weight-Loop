...
for dataset in datasets:
    # 读取TSV文件
    file_path = f'png_tsvs/ret/{dataset}_ret_3.tsv'  # 替换成你的TSV文件路径
    ...
    # 转换DataFrame为长格式
    df_long = df.melt(id_vars=['Method'], var_name='loop', value_name='Acc@5')
    ...
    sns.lineplot(data=df_long, x='loop', y='Acc@5', hue='Method', palette="deep",linewidth=2.5)
    ...
    plt.show()  # 显示图表
    plt.savefig(f'png_tsvs/ret/{dataset}_ret.png', bbox_inches='tight')  # 保存图表到文件
