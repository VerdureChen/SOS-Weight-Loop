...
from eva_percentage import calculate_llm_human_proportion, calculate_llm_type_rate, calculate_query_right_num
...
def get_args():
    ...
    parser.add_argument('--weight_merge_path', type=str, default=None, help='weight merge path')
    parser.add_argument('--retrieval_model_names', type=str, default='bm25', help='retrieval model names')
...
for retrieve_model_name in retrieve_model_names:
    for weight_type in weifht_types:
        retrieval_order_dict[f'{retrieve_model_name}+{weight_type}'] = {}
...
def evaluate_llm_type_rate(retrieval_file, topk=None, llm_names=generate_model_names):
    if topk is None:
        topk = [5, 20, 50]
    print('evaluate percentage...')
    percentage = calculate_llm_type_rate(retrieval_file, topk, llm_names)
    return percentage

def evaluate_llm_query_right_num(retrieval_file, topk=None, llm_names=generate_model_names):
    if topk is None:
        topk = [5, 20, 50]
    print('evaluate query correctness...')
    percentage = calculate_query_right_num(retrieval_file, topk, llm_names)
    return percentage
...
def extract_method_name(file_name, directory_name):
    ...
    filter_type = None
    weight_type = ''
    if directory_name in file_name and not file_name.endswith(('.trec', '.jsonl', 'pkl')):
        ...
        if 'after' in file_name:
            # get "after_init" or "after_update" by regex
            weight_type = re.findall(r'after_\w+', file_name)[0].split('_')[1]

        return [generate_model_name, retrieval_method_name, rerank_model_name, ref_num, loop_num, filter_type, weight_type]
    return None
...
def format_tsv_output(directory_name, results, task, output_file_name=None):
    ...
    with open(output_file_name, 'w') as f:
        if task in ['retrieval', 'filter_bleu_retrieval', 'filter_source_retrieval']:
                    ...
                    for gram in grams:
                        ...
                        with open(f_path, 'w') as file:
                            ...
                            for method, loop_nums in results.items():
                                if results[method] == {}:
                                    continue
                                header = "ref_num\tgram\tmethod\t" + "\t".join([f"{i}" for i in range(0, 11)]) + "\n"
                                file.write(header)
                                row_data = [f"{ref_num}\t{gram}\t{method}"]
                                # 对于每个循环，获取相应的bleu
                                for loop_num in range(0, 11):
                                ...
                                # 写入空行
                                file.write("\n")
        elif task in ['correctness_query']:
            print(results)
            # 对每个topk和ref_num组合创建一个文件
            # 从bleu_order_dict中获取所有可能的ref_num和topk值
            ref_nums = set()
            ...
            for ref_num in ref_nums:
                for topk in topks:
                    f_name = f"correctness_top{topk}_ref{ref_num}.tsv"
                    f_path = os.path.join(os.path.dirname(output_file_name), f_name)
                    with open(f_path, 'w') as file:
                        # 写入表头
                        header = "ref_num\tgram\tmethod\t" + "\t".join([f"{i}" for i in range(0, 11)]) + "\n"
                        ...
                        for method, loop_nums in results.items():
                            if results[method] == {}:
                                continue
                            # 遍历所有gram值，从小到大
                            for gram in range(0, topk+1):
                                row_data = [f"{ref_num}\t{gram}\t{method}"]
                                # 对于每个循环，获取相应的bleu
                                for loop_num in range(0, 11):
                                    score = loop_nums.get(str(loop_num), {}).get(str(ref_num), {}).get(topk, {}).get(
                                        gram, 0)
                                    row_data.append(str(score))
                                # 写入一行数据
                                file.write("\t".join(row_data) + "\n")
                    ...
                    with open(f_path, 'w') as file:
                        ...
                        for method, generate_models in results.items():
                            for generate_model_name, loop_data in generate_models.items():
                                ...
                                # 写入一行数据
                                file.write("\t".join(row_data) + "\n")
        elif task in ['llm_text_type_rate']:
            # print(results)
            # 对每个topk和ref_num组合创建一个文件
            # 从percentage_order_dict中获取所有可能的ref_num和topk值
            ref_nums = set()
                ...
                for generate_model_name, loops in generate_models.items():
                    for loop_num, refs in loops.items():
                        for ref_num, topk_scores in refs.items():
                            ref_nums.add(ref_num)
                            for key in topk_scores.keys():
                                topks.add(key.split('_')[0])
            print(results)
            topk_values = sorted(list(topks), key=lambda x: int(x))

            for ref_num in ref_nums:
                for topk in topk_values:
                    f_name = f"llm_type_rate_top{topk}_ref{ref_num}.tsv"
                    f_path = os.path.join(os.path.dirname(output_file_name), f_name)
                    with open(f_path, 'w', encoding='utf-8', newline='') as tsvfile:
                        writer = csv.writer(tsvfile, delimiter='\t')

                        # 写入表头
                        header = ["Metric/Ref"] + [f"Loop {i}" for i in range(1, 11)]
                        ...
                        for method, generate_models in results.items():
                            for generate_model_name, loops in generate_models.items():
                                metrics_data = {
                                    'total_true': [],
                                    'total_false': [],
                                    'total_has_answer_true_proportion': [],
                                    'total_has_answer_false_proportion': []
                                }

                                # 收集每个 loop 的数据
                                for loop_num in range(0, 10):
                                    loop_data = loops.get(str(loop_num), {}).get(str(ref_num), {})
                                    # print(loop_data)
                                    # print(f"{topk}_{generate_model_name}_true")
                                    # print(loop_data.keys())
                                    metrics_data['total_true'].append(loop_data.get(f"{topk}_{generate_model_name}_true", 0))
                                    metrics_data['total_false'].append(loop_data.get(f"{topk}_{generate_model_name}_false", 0))
                                    metrics_data['total_has_answer_true_proportion'].append(
                                        loop_data.get(f"{topk}_{generate_model_name}_has_answer_true_proportion", 0))
                                    metrics_data['total_has_answer_false_proportion'].append(
                                        loop_data.get(f"{topk}_{generate_model_name}_has_answer_false_proportion", 0))

                                # 写入模型名称作为标题
                                model_title = f"{method}_{generate_model_name}"
                                writer.writerow([model_title])
...
def compute_res(directory, task, elasticsearch_url=None, index_name=None, api_key=None, api_base=None, weight_merge_path=None):
    ...
    if task == 'bleu':
        ...
        if weight_merge_path is None:
            total_path = os.path.dirname(directory)
            weight=False
        else:
            total_path = os.path.join(weight_merge_path, retrieve_model_names[0])
            weight=True
        llm_data = gather_LLM_gen_text(total_path, weight)
    ...
    for file_name in tqdm(os.listdir(directory)):
        ...
        if method_name:
            if method_name[0] is None and task in ['retrieval', 'filter_bleu_retrieval', 'filter_source_retrieval']:
                # retrieval file
                generate_model_name, retrieval_method_name, rerank_model_name, ref_num, loop_num, file_type, weight_type = method_name
                ...
                if rerank_model_name is not None:
                    method = f'{retrieval_method_name}+{rerank_model_name}+{weight_type}'
                else:
                    method = f'{retrieval_method_name}+{weight_type}'
            ...
            elif method_name[0] is None and task in ['correctness_query']:
                # percentage file
                generate_model_name, retrieval_method_name, rerank_model_name, ref_num, loop_num, file_type, weight_type = method_name

                file_path = os.path.join(directory, file_name)
                print(file_path)
                scores = evaluate_llm_query_right_num(file_path)
                # score format like {5: {'human': 100, 'human_proportion': 0.5, 'gpt-3.5-turbo': 100, 'gpt-3.5-turbo_proportion': 0.5}}
                ...
                if bleu_order_dict[method].get(loop_num) is None:
                    bleu_order_dict[method][loop_num] = {}
                bleu_order_dict[method][loop_num][ref_num] = scores
            elif method_name[0] is None and task in ['llm_text_type_rate']:
                # percentage file
                generate_model_name, retrieval_method_name, rerank_model_name, ref_num, loop_num, file_type, weight_type = method_name


                if task == 'llm_text_type_rate' and file_type != None:
                    continue

                file_path = os.path.join(directory, file_name)
                print(file_path)
                scores = evaluate_llm_type_rate(file_path)
                # score format like {5: {'human': 100, 'human_proportion': 0.5, 'gpt-3.5-turbo': 100, 'gpt-3.5-turbo_proportion': 0.5}}
                if rerank_model_name is not None:
                    method = f'{retrieval_method_name}+{rerank_model_name}'
                else:
                ...
                for generate_model in generate_model_names + ['human', 'llms_total']:
                    if percentage_order_dict[method].get(generate_model) is None:
                        percentage_order_dict[method][generate_model] = {}
                    if percentage_order_dict[method][generate_model].get(loop_num) is None:
                        percentage_order_dict[method][generate_model][loop_num] = {}
                    if percentage_order_dict[method][generate_model][loop_num].get(ref_num) is None:
                        percentage_order_dict[method][generate_model][loop_num][ref_num] = {}
                    # 遍历每个 top-k 值
                    for topk, score_data in scores.items():
                        true_prop_key = f'{generate_model}_has_answer_true_proportion'
                        false_prop_key = f'{generate_model}_has_answer_false_proportion'
                        true_count_key = f'{generate_model}_true'
                        false_count_key = f'{generate_model}_false'
                        percentage_order_dict[method][generate_model][loop_num][ref_num][f'{topk}_{true_prop_key}'] = score_data.get(true_prop_key, 0)
                        percentage_order_dict[method][generate_model][loop_num][ref_num][f'{topk}_{false_prop_key}'] = score_data.get(false_prop_key, 0)
                        percentage_order_dict[method][generate_model][loop_num][ref_num][f'{topk}_{true_count_key}'] = score_data.get(true_count_key, 0)
                        percentage_order_dict[method][generate_model][loop_num][ref_num][f'{topk}_{false_count_key}'] = score_data.get(false_count_key, 0)
    ...
    elif task in ['bleu', 'filter_bleu_bleu', 'filter_source_bleu', 'correctness_query']:
        format_tsv_output(directory_name, bleu_order_dict, task, output_file_name)
    elif task in ['percentage', 'filter_bleu_percentage', 'filter_source_percentage', 'llm_text_type_rate']:
...
if __name__ == '__main__':
    ...
    compute_res(args.directory, args.task, args.elasticsearch_url, args.index_name, args.api_key, args.api_base, args.weight_merge_path)
