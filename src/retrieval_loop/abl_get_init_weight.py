...
def process_document(document, input_file, retrieved_file, retrieval_model, normalize_embeddings, llm_detector_model,
    ...
    def process(doc):
        ...
        # 选择最佳解决方案
        best_solution = select_best_solution(pareto_front, fit_weights)
        ...
        w = sigmoid(
            1 + alpha * sim_scores[qid][qid] - beta * self_bleu_scores - gamma * total_llm_probs[qid][qid] + delta *
            fact_score[qid])
...
def main(config_path: str):
    ...
    if "fit_weights" in config:
        ...
        if fit == "9":
            ...
            fit_weights = [1,1,1,1]
            fit_index = fit
            fit_weights[int(fit_index)] = 0.25
