...
class Batch_FAISS(FAISS):
    ...
    """
        ...
        # pretty print the time y-m-d h:m:s
        print(f'start searching, query size: {len(vectors)}, time: {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}')

        scores, indices = self.index.search(vectors, k if filter is None else fetch_k)
        results = []
        print(f'start fetching, query size: {len(indices)}, time: {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}')
        for idx, query_indices in enumerate(indices):
            docs = []
