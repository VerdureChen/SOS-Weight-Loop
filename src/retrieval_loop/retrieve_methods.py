...
def Retrieval(query_files, page_content_column, retrieval_model, index_name, index_path, normalize_embeddings,
              output_files, elasticsearch_url, with_alpha):
    ...
    if type(page_content_column) == str:
        page_content_column = [page_content_column]*len(query_files)

    for query_file, output_file, page_column in zip(query_files, output_files, page_content_column):
        # load queries
        loader = HuggingFaceDatasetLoader('json', data_files=query_file,
                                          page_content_column=page_column)
        print(f'page_content_column: {page_column}')
        ...
        with open(output_file_trec, 'w', encoding='utf-8') as f_t:
            ...
            for batch in tqdm(batch_queries):
                ...
                if 'answers' in batch_metadata[0]:
                    batch_answers = [metadata['answers'] for metadata in batch_metadata]
                else:
                ...
                if "DPR" in retrieval_model.upper():
                    ...
                    if with_alpha:
                        # 追加 0 以匹配索引嵌入的维度
                        embedded_queries = np.array(embedded_queries, dtype=np.float64)
                        embedded_queries = np.hstack(
                            [embedded_queries, np.zeros((embedded_queries.shape[0], 1), dtype=np.float64)])
