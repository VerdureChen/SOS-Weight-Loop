...
def get_args():
    ...
    if args.overrides:
        ...
        try:
            # strip the line breaks and spaces from the string
            args.overrides = json.loads(args.overrides.replace('\n', '').replace(' ', ''))
            print(f"Overriding config with: {args.overrides}")
        except json.JSONDecodeError:
            args.overrides = args.overrides.replace('\n', '').replace(' ', '')
            print("Overrides must be a valid JSON string")
            print(f"Got: {args.overrides}")
            sys.exit(1)
...
def get_template_path(stage_name, method_name, data_name):
    ...
    elif stage_name == 'weight_init':
        template_path = f'weight_configs/{method_name}-config-{data_name}.json'
    elif stage_name == 'weight_update':
        template_path = f'weight_configs/{method_name}-config-update.json'
    elif stage_name == 'alpha_index':
        template_path = f'alpha_index_configs/{method_name}-config-{data_name}.json'
    elif stage_name == 'alpha_retrieval':
        template_path = f'alpha_retrieval_configs/{method_name}-config-{data_name}.json'
    elif stage_name == 'compute_total_llm':
        template_path = f'merge_configs/{method_name}-config-{data_name}-llm.json'
    elif stage_name == 'merge':
        template_path = f'merge_configs/{method_name}-config-{data_name}-merge.json'
    elif stage_name == 'pq_llm':
        template_path = f'merge_configs/{method_name}-config-{data_name}-pq.json'
    else:
